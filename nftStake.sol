...
contract NftStake is IERC721Receiver, ReentrancyGuard {
 ...
 function _stakeRemove(StakeStore[] storage stakeListRef, uint256 stakeIndex)
    ...
    function unStakeNFT(uint256 tokenId, uint256 stakeListIndex) public nonReentrant returns (bool) {
        return _unStakeNFT(tokenId, stakeListIndex);
    }

    function _unStakeNFT(uint256 tokenId, uint256 stakeListIndex) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        delete receipt[tokenId];
         _stakeRemove(stakeListRef, stakeListIndex);
        // return token
